...
public class SmartStratPlayerDeep extends HolsDerGeierSpieler {
	...
	public int gibKarte(int aktuelleTierkarte){
		...
		DefaultMutableTreeNode root = builtTree(_animalCards,_playerACards,_playerBCards,_pointDelta, _drawPoints,aktuelleTierkarte);
		TransformedNormalForm tnf = ((DataHull)root.getUserObject()).getTransformedNormalForm();
	...
	public SmartStratPlayerDeep(){
		
	}
	
	public DefaultMutableTreeNode builtTree(ArrayList<Integer> animalCards,
			ArrayList<Integer> playerACards, ArrayList<Integer> playerBCards, Integer pointDelta,
			Integer drawPoints, Integer currentAnimalCard){
		...
		infoCopy.getAnimalCardDeck().remove(currentAnimalCard);
		infoCopy.setCurrentAnimalCard(new Integer(currentAnimalCard.intValue()));
		DataHull dataHull = new DataHull();
		dataHull.setInfoLevel(infoCopy);
		DefaultMutableTreeNode root = new DefaultMutableTreeNode();
		root.setUserObject(dataHull);
		builtChildrenRandom(root,0,1);
		getPGameValueRandom(root);
		return root;
	}
	...
	public void builtChildrenRandom(DefaultMutableTreeNode dmtn,Integer oldValue, Integer depth){
		InfoLevel original = ((DataHull)dmtn.getUserObject()).getInfoLevel();
		InfoLevel kopie = cloneInfoLevel(original);
		Integer bewertungsZiffer = bewerte(kopie);
		System.out.println("depth:"+ depth);
		System.out.println("ziffer: " + bewertungsZiffer);
		System.out.println();
		System.out.println("AnimalCardSize: " + _animalCards.size());
		int depthToGo = 1;
		
		if(_animalCards.size()>=3)
			depthToGo = 5;
		if(_animalCards.size()>=4)
			depthToGo = 5;
		if(_animalCards.size()>=6)
			depthToGo = 5;
		if(_animalCards.size()>=7)
			depthToGo=5;
		if(_animalCards.size()>=9)
			depthToGo=4;
		if(_animalCards.size()>=11)
			depthToGo=3;
		if(_animalCards.size()>=13)
			depthToGo=2;
		System.out.println("DepthToGo: " + depthToGo);
		System.out.println("Level: " + dmtn.getLevel());
		if(((depth>=depthToGo||bewertungsZiffer<0||bewertungsZiffer>4||bewertungsZiffer< oldValue)&&dmtn.getLevel()>=1)){
			...
			for(int j=0;j<original.getPlayerBDeck().size();j++){
				...
				if(infoLevel.getAnimalCardDeck().size()>0){
					for(int k=0;k < infoLevel.getAnimalCardDeck().size();k++){
						...
						dmtn.add(child);
						builtChildrenRandom(child,bewertungsZiffer,(depth+1));
					}
				}
				else{
					DataHull dataHull = new DataHull();
	...
	public Double getPGameValueRandom(DefaultMutableTreeNode node){
		...
		TransformedNormalForm tnf = new TransformedNormalForm(nf,1000000);
	...
	public int bewerte(InfoLevel infoLevel){
		...
		for(int i=0; i<animalCardsSorted.size();i++){
			if(animalCardsSorted.get(i)<0){
				number = animalCardsSorted.get(i);
				animalCardsSorted.remove(number);
				animalCardsSorted.add((-1)*(number));
			}
		}
